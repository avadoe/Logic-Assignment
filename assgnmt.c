...
#define MAX 1000 // Maximum size of the input expression & the stack
#define COUNT 10
...
int evaluateSubTree(TreeNode *root)
{
  ...
  else if (root->left == NULL && root->right == NULL)
  {
    ...
    if (hasOcurred(root->data) == -1)
    {
      printf("Enter Truth Value (0 or 1) of %c: ", root->data);
      int c;
      scanf("%d", &c);
      Push(root->data, &top, operands);
      truthvalues[top] = c;
      return c;
    }
    else
    {
      return truthvalues[hasOcurred(root->data)];
    }
  }
  else
  {
